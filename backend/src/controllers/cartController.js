...
const addToCart = async (req, res) => {
  try {
    ...
    const listing = await query(`SELECT * FROM crop_listings WHERE id = $1 AND is_available = TRUE AND is_active = TRUE`, [listing_id]);
    ...
    await query(
      `INSERT INTO cart_items (consumer_id, listing_id, quantity) VALUES ($1,$2,$3)
       ON CONFLICT (consumer_id, listing_id) DO UPDATE SET quantity = $3, updated_at = NOW()`,
...
const removeFromCart = async (req, res) => {
  try {
    await query(`DELETE FROM cart_items WHERE id = $1 AND consumer_id = $2`, [req.params.id, req.user.id]);
