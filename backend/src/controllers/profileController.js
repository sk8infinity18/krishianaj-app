...
const getFarmerProfile = async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT first_name, last_name, farmer_id, farm_name, farm_location, farm_state, farm_district,
              profile_image, total_earnings, rating, total_reviews
       FROM farmers WHERE farmer_id = $1`, [id]);
...
const updateFarmerProfile = async (req, res) => {
  try {
    const { farm_name, farm_location, farm_state, farm_district } = req.body;
    ...
    const result = await query(
      `UPDATE farmers SET farm_name = COALESCE($1, farm_name), farm_location = COALESCE($2, farm_location),
       ...
       WHERE farmer_id = $6 RETURNING first_name, last_name, farmer_id, farm_name, farm_location, farm_state, farm_district, profile_image`,
      [farm_name||null, farm_location||null, farm_state||null, farm_district||null, profile_image, req.user.id]
    );
...
const getConsumerProfile = async (req, res) => {
  try {
    const result = await query(
      `SELECT first_name, last_name, consumer_id, profile_image, delivery_address, delivery_city, delivery_state, delivery_pincode
       FROM consumers WHERE consumer_id = $1`, [req.user.id]);
  ...
  try {
    ...
    const result = await query(
      `UPDATE consumers SET delivery_address = COALESCE($1, delivery_address), delivery_city = COALESCE($2, delivery_city),
        ...
        profile_image = COALESCE($5, profile_image)
       WHERE consumer_id = $6 RETURNING first_name, last_name, consumer_id, delivery_address, delivery_city, delivery_state, delivery_pincode, profile_image`,
