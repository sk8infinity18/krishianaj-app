  ...
  try {
    ...
    const result = await query(
      `UPDATE farmers SET farm_name = COALESCE($1, farm_name), farm_location = COALESCE($2, farm_location),
       ...
       WHERE id = $7 RETURNING id, first_name, last_name, farm_name, farm_state, farm_district, bio, profile_image`,
  ...
  try {
    ...
    const result = await query(
      `UPDATE consumers SET delivery_address = COALESCE($1, delivery_address), delivery_city = COALESCE($2, delivery_city),
       ...
       WHERE id = $7 RETURNING id, first_name, last_name, phone_number, email, delivery_address, delivery_city, delivery_state, delivery_pincode, profile_image`,
